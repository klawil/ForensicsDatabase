...
<?php echo Students(0,"StudentSelect"); ?>
<input type="submit" value="select">
</form>
<?php
if ( isset($_POST['SID']) && $ErrorString == "" ) {
	$query = mysql_query("select distinct EName from Events, Results where Results.EID = Events.EID and Results.SID = '" . $_POST['SID'] . "' and Results.State = 1 order by EName;");
...
";
	...
	echo "<h3>Progress</h3>
<div id='visualization' style='width: 95%;'></div>
<script>
function MakeGraph() {
...
";
	...
	while ( $CurrentRow <= $NumRows ) {
		$Data = mysql_fetch_assoc($query);
		$Events[$Data['EName']] = $CurrentRow;
		echo "data.addColumn('number', '" . $Data['EName'] . "');
";
		$CurrentRow++;
	}
...
";
	...
	$query = mysql_query("select EName, Date, avg(10 - PRanks/NumberRounds) as Ranks from Results, Tournaments, Events where Results.TID = Tournaments.TID and Results.EID = Events.EID and (Results.SID='" . $_POST['SID'] . "' or Results.SID2='" . $_POST['SID'] . "') group by Date, EName order by Date, EName;");
	if (( mysql_errno() )) {
		$ErrorString = "Error - MySQL error " . mysql_errno() . ": " . mysql_error() . ".";
		echo $ErrorString;
		break;
...
";
	...
	chart.draw(data, {height: 500, vAxis: { viewWindowMode:'explicit', viewWindow: {min: 0, max: 9}}, interpolateNulls: true});
	}
